...
const events = [
  ...
  { category: "DEVELOPER TOOLS", title: "The agent stack is settling around durable primitives", summary: "Three new releases point to the same pattern: traces, permissions, and resumable work before clever autonomy.", sourceCount: 5, age: "2 hr ago", relevance: "You follow AI agents and developer infrastructure", signal: "PATTERN" },
  { category: "RESEARCH", title: "A benchmark result worth reading past the headline", summary: "The reported gain is real, but the evaluation conditions make it narrower than the first wave of coverage suggests.", sourceCount: 4, age: "5 hr ago", relevance: "Research depth: advanced · evidence available", signal: "EVIDENCE" },
];
  ...
  return (
    <main className="app-shell">
      ...
      <section className="content" id="feed">
        ...
        <div className="event-list">{events.map((event, index) => <article className={`event-card ${index === 0 ? "featured" : ""}`} key={event.title}><div className="event-number">0{index + 1}</div><div className="event-body"><div className="event-meta"><span className="category">{event.category}</span><span>{event.age}</span><span>{event.sourceCount} sources</span></div><h3>{event.title}</h3><p className="event-summary">{event.summary}</p><div className="event-footer"><span className="relevance"><span className="spark">✦</span>{event.relevance}</span><span className="signal-chip">{event.signal}</span><button className="brief-link" type="button">Read brief <span>↗</span></button></div></div></article>)}</div>
